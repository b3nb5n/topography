...
export const getTokenHandler: RequestHandler<{}, GetTokenResponse> = async (
	req,
	res
) => {
	...
	try {
		const user = (
			await userRepository.find({ where: { 'data.email': data.email } })
		)[0]
		if (!user) return res.status(404).send({ error: 'unknown account identifier' })

		const passwordMatches = await bcrypt.compare(data.password, user.data.password)
		if (!passwordMatches)
			return res.status(406).send({ error: 'incorrect password' })

		const payload: Payload = { uid: user.id.toString() }
